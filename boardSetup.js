let numOfShips = 5;//prompt("Please enter the number of ships you want")
let squareSize = 40;
    ...
    for (let i = 0; i < 11; i++)
    {
        for (let j = 0; j < 11; j++) 
        {
            ...
            if(!(i==0 || j==0))
            {
                ...
                square.addEventListener('click', ()=> placingShips(square))
...
const renderSecondBoard = () => {
    ...
    {   
        for(let j = 0; j < 11; j++)
        {
            ...
            gameBoardContainer2.appendChild(box)
            if(!(i==0 || j==0))
            {
                box.i = i-1
                box.j = j-1

                box.addEventListener('click', ()=> placingShips(box))
